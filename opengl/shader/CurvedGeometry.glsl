...
void main()
{
    ...
    gl_Position = projectionMatrix *viewMatrix * modelMatrix * vec4(vectex1, 1.0);
    EmitVertex();

    vec3 vectex2 = gl_in[0].gl_Position.xyz + normal*width;
    gl_Position = projectionMatrix *viewMatrix * modelMatrix * vec4(vectex2, 1.0);
    EmitVertex();

    vec3 vectex3 = gl_in[1].gl_Position.xyz - normal*width;
    gl_Position = projectionMatrix *viewMatrix * modelMatrix * vec4(vectex3, 1.0);
    EmitVertex();

    vec3 vectex4 = gl_in[1].gl_Position.xyz + normal*width;
    gl_Position = projectionMatrix *viewMatrix * modelMatrix * vec4(vectex4, 1.0);
    EmitVertex();
